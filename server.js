...
// Start the Server
const PORT = process.env.PORT || 10000; // Adjust to match your environment
app.listen(PORT, () => {
  console.log(`Server running on http://localhost:${PORT}`);
});

// Login the Bot
client.login(process.env.BOT_TOKEN).catch(err => console.error('Bot login failed:', err));require('dotenv').config();
const express = require('express');
const { Client, GatewayIntentBits } = require('discord.js');
const { initializeApp } = require('firebase/app');
const { getFirestore, doc, setDoc, getDoc, collection, getDocs } = require('firebase/firestore');
